...
def learn(tensorboard: SummaryWriter, optimizer: optim.Optimizer, learning_net: nn.Module, replay_buffer: ReplayBuffer,
          batch_size: int, policy_weight: float, entropy_weight:float, ppo_epsilon: float, gradient_clipping: float,
          normalize_advantage: bool, global_step: int):
    """
    ...
    Args:
        ...
        global_step: The current global step for reporting to tensorboard.
    """
    ...
    if enable_crashing_tensorboard:
        ...
        if masked_reward.size()[0]:
            tensorboard.add_histogram("reward/train",
...
def ppo(hparams: Dict, time_limit_secs=None, early_stopper=None):
    ...
    if hparams["optimizer"] == "adam":
        ...
        optimizer = optim.SGD(learning_net.parameters(), lr=hparams["sgd_lr"], momentum=hparams["sgd_momentum"],
                              nesterov=True)
    else:
        assert False

    # Total number of gradient descent steps we've taken.
    global_step = 0
    replay_buffer_size = 10000
    if hparams["normalize_observations"]:
        # TODO(anyone): decide on gamma / make it a hyperparameter.
        replay_buffer = NormalizingReplayBuffer(replay_buffer_size, 0, DEFAULT_PLAYER_ENCODING, DEFAULT_CARDS_ENCODING)
    else:
        ...
        if len(replay_buffer) >= batch_size:
            for i in range(hparams["ppo_epochs"]):
                learn(tensorboard, optimizer, learning_net, replay_buffer, batch_size, hparams["policy_weight"],
                      hparams["entropy_weight"], hparams["ppo_epsilon"], hparams["gradient_clipping"],
                      hparams["normalize_advantage"],
                      global_step)
                global_step += 1
            replay_buffer.clear()
