...
class SameTypeAdvantage(CardHeuristic):
    ...
    def modification_value(self, player: 'Player', card: 'MonsterCard'):
        return len([player_card for player_card in itertools.chain(player.in_play, player.hand) if
                    player_card.check_type(card.monster_type)])
...
class MamasLove(CardHeuristic):
    ...
    def modification_value(self, player: 'Player', card: 'MonsterCard'):
        if type(card) == MamaBear:
            return 20
        return 10 * len([player_card for player_card in itertools.chain(player.in_play, player.hand) if
                         type(player_card) == MamaBear])
...
class DragonPayoffs(CardHeuristic):
    ...
    def modification_value(self, player: 'Player', card: 'MonsterCard'):
        return 50 if len([player_card for player_card in itertools.chain(player.in_play, player.hand) if
                          player_card.check_type(MONSTER_TYPES.DRAGON)]) >= 3 else 0
...
class MonstrousMacawPower(CardHeuristic):
    ...
    def modification_value(self, player: 'Player', card: 'MonsterCard'):
        deathrattler_tiers = [player_card.tier for player_card in itertools.chain(player.in_play, player.hand) if
                              player_card.deathrattles]
...
class Scavengers(CardHeuristic):
    ...
    def modification_value(self, player: 'Player', card: 'MonsterCard'):
        if player.tavern_tier < 3:
            return len([player_card for player_card in itertools.chain(player.in_play, player.hand) if
                        player_card.check_type(MONSTER_TYPES.BEAST)])
